countmotif2 <- function(x, motif, z, p, JP, JZ, P, Q, R, Z, Y, X = NULL, dP, jP, dZ, jZ, J3 = NULL, MA = NULL, MB = NULL, MC = NULL, MD = NULL, Na = NULL, NB = NULL, NC = NULL){
  if(motif == 1){
    sum(x)
  } else if(motif == 2){
    sum(dZ * (dZ - jZ)) / 2
    ...
    sum(P * (P - JP) * (P - 2 * JP)) / 12 - sum(dP * (dP - jP) * (dP - 2 * jP)) / 3
  } else if(motif == 13){
    sum(Z * Y * (Y - JZ)) / 2
  } else if(motif == 14){
    sum(Z * Y * Z) / 2
