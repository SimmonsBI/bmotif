normalise_positions <- function(pc,type){
  ...
  if(type == "size class"){
    ...
    pc[,paste0("p",1:2)] <- pc[,paste0("p",1:2)]/sapply(1:nrow(pc), function(x) sum(pc[x,paste0("p",1:2)]))
    pc[,paste0("p",3:6)] <- pc[,paste0("p",3:6)]/sapply(1:nrow(pc), function(x) sum(pc[x,paste0("p",3:6)]))
    pc[,paste0("p",7:16)] <- pc[,paste0("p",7:16)]/sapply(1:nrow(pc), function(x) sum(pc[x,paste0("p",7:16)]))
    pc[,paste0("p",17:46)] <- pc[,paste0("p",17:46)]/sapply(1:nrow(pc), function(x) sum(pc[x,paste0("p",17:46)]))
    if(ncol(pc) == 148){
      # pc[,paste0("p",47:148)] <- pc[,paste0("p",47:148)]/apply(pc[,paste0("p",47:148)], 1, sum)
      pc[,paste0("p",47:148)] <- pc[,paste0("p",47:148)]/sapply(1:nrow(pc), function(x) sum(pc[x,paste0("p",47:148)]))
    }
  } else if(type == "sum"){
    # pc[,paste0("p",1:ncol(pc))] <- pc[,paste0("p",1:ncol(pc))]/apply(pc[,paste0("p",1:ncol(pc))], 1, sum)
    pc[,paste0("p",1:ncol(pc))] <- pc[,paste0("p",1:ncol(pc))]/sapply(1:nrow(pc), function(x) sum(pc[x,]))
  } else {
