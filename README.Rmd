...
`bmotif` considers all 44 unique bipartite motifs up to six nodes. Within these motifs there are 148 unique node positions.

`bmotif` has two functions: `mcount` and `node_positions`. `mcount` counts occurrences of motifs in a bipartite network. `node_positions` counts the number of times each node in a network occurs in each of the positions within the motifs.
