...
test_that("'sizeclass_NAzero' replaces all NAs/NaNs with zero",{
  for(i in 1:10){
    ...
    np_normal <- normalise_node_positions(np, type = "sizeclass")
    np_NAzero <- normalise_node_positions(np, type = "sizeclass_NAzero")
...
test_that("'levelsize_NAzero' replaces all NAs/NaNs with zero",{
  for(i in 1:10){
    ...
    np <- node_positions(M = m, six_node = TRUE, level = "all", weights_method = "none", weights_combine = "none", normalisation = "none")
    np_normal <- normalise_node_positions(np, type = "levelsize")
    np_NAzero <- normalise_node_positions(np, type = "levelsize_NAzero")
    expect_identical(all(np_NAzero[is.na(np_normal)] == 0), TRUE)
  }
})
...
test_that("'motif_NAzero' replaces all NAs/NaNs with zero",{
  for(i in 1:10){
    m <- motifs[[sample(44,size = 1)]]
    np <- node_positions(M = m, six_node = TRUE, level = "all", weights_method = "none", weights_combine = "none", normalisation = "none")
    np_normal <- normalise_node_positions(np, type = "motif")
    np_NAzero <- normalise_node_positions(np, type = "motif_NAzero")
    expect_identical(all(np_NAzero[is.na(np_normal)] == 0), TRUE)
  }
})
...
test_that("'sizeclass_plus1' removes all NAs/NaNs",{
  for(i in 1:10){
    ...
    np <- node_positions(M = m, six_node = TRUE, level = "all", weights_method = "none", weights_combine = "none", normalisation = "none")
    np_normal <- normalise_node_positions(np, type = "sizeclass")
    np_NAzero <- normalise_node_positions(np, type = "sizeclass_plus1")
...
test_that("'levelsize_plus1' removes all NAs/NaNs",{
  for(i in 1:10){
    ...
    np <- node_positions(M = m, six_node = TRUE, level = "all", weights_method = "none", weights_combine = "none", normalisation = "none")
    np_normal <- normalise_node_positions(np, type = "levelsize")
    np_NAzero <- normalise_node_positions(np, type = "levelsize_plus1")
    expect_identical(all(!is.na(np_NAzero[is.na(np_normal)])), TRUE)
  }
})
...
test_that("'motif_plus1' removes all NAs/NaNs",{
  for(i in 1:10){
    ...
    np <- node_positions(M = m, six_node = TRUE, level = "all", weights_method = "none", weights_combine = "none", normalisation = "none")
    np_normal <- normalise_node_positions(np, type = "motif")
    np_NAzero <- normalise_node_positions(np, type = "motif_plus1")
    expect_identical(all(!is.na(np_NAzero[is.na(np_normal)])), TRUE)
  }
})
