...
motif_sd_minors <- function(W) {
  ...
  msd <- sqrt(zerodiv_vec(sd_sum, mc))
  msd
}
...
test_that("Testing simple version of M2", {
  ...
  expect_equal(msd[2:17], rep(0,16))
})
...
test_that("Testing simple version of M3", {
  ...
  expect_equal(msd[2:3], c(0,0))
  expect_equal(msd[4:17], rep(0,14))
...
test_that("Test: If mcount is zero, sd should be zero", {
  for (i in 1:10) {
    W <- rwm(10,10, 0.1)
    if(sum(W) != 0) {
